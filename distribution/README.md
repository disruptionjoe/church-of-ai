# Distribution

Distribution in V1 is content-only. It helps people find the public map,
updates, and repo sequence without opening contribution, governance,
membership, rewards, or proving-ground participation.

## Current State

The 2026-07-18 distribution proposal is admitted as a working production
strategy, not an external-action authorization.

Church of AI owns the public voice, channel fit, and claim-status discipline.
The Church of AI / Drafting Factory relationship is accepted in
[`drafting-factory-interface.md`](drafting-factory-interface.md). The Factory
may maintain local candidate and ready-to-post inventory against that interface.
Joe performs any account setup and all posting.

The latest source-owner recheck is
[`source-owner-recheck-2026-07-22.md`](source-owner-recheck-2026-07-22.md).
It finds that Factory commit `6008934` does not clear the existing Church
corrections; all three drafts remain outside Church-ready inventory.

## Candidate Channel Fit

- Substack: active owned-audience home for plain-language maps and dated
  updates.
- Instagram Reels: active internal short-video production channel and easiest
  starting point when Joe elects to create and use the account.
- YouTube Shorts: active internal mirror channel for evergreen, searchable
  clips when Joe elects to create and use the account.
- TikTok: held optional mirror with higher policy volatility and weaker
  audience fit.
- X: out.

The efficient video pattern is one vertical clip that can be reused across the
chosen short-video channels.

These are internal production postures, not a live publishing plan, channel
launch, or request for Joe to create accounts.

## Content Rules

- Use public Church and linked-repo content only.
- Preserve claim status: proven so far, being tested, hypothesized, or
  deferred.
- Allowed calls to action: read the map, follow the work, or read the update.
- Not allowed yet: come contribute, join, govern, claim membership, rewards,
  DAO/token framing, or proving-ground participation.
- Church explains and invites; it does not certify research truth or decide for
  other repositories.

## Setup Boundary

Agents may plan, draft repo-local content, and prepare mailbox proposals. Agents
may not create accounts, post, schedule, send, publish outside GitHub, spend, or
change external services. Those actions require Joe's explicit scoped approval,
and Joe performs them himself.
